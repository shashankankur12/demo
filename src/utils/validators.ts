...
export const validateRequiredGender = () =>
  yup.string().required('validation.gender.required');

export const validateRequiredCredentials = () =>
  yup.string().required('validation.credentials.required');
...
export const validateDateRequired = () =>
  yup.string().required('validation.dob.required');

export const validateAddress = (messages?: {
  required?: LocaleString;
  inValid?: LocaleString;
}) =>
  yup
    .string()
    // .matches(alphaNumeric, messages?.inValid || 'validation.address.invalid')
    .max(60, 'validation.address.max')
    .required(messages?.required || 'validation.address.required');
